...
void CommandQueue::init(ComPtr<ID3D12Device> d) {
	device = d;
...
CommandList CommandQueue::getCommandList() {
	...
	RETURN_ON_ERROR(
		cmdList.reset(cmdAllocator.Get()), cmdList,
		"Failed to reset the command list!"
	);

	RETURN_ON_ERROR(
		cmdList.setPrivateData(__uuidof(ID3D12CommandAllocator), cmdAllocator.Get()), cmdList,
...
FenceValue CommandQueue::executeCommandLists() {
	...
	// Lock the pending command lists queue until the command lists and any needed resource barriers are exctracted
	{
		...
		if (resBarriers.size() > 0) {
			...
			cmdListPendingBarriers.resourceBarriers(resBarriers);
...
FenceValue CommandQueue::signal() {
	FenceValue fenceVal = ++fenceValue;
	RETURN_ON_ERROR(commandQueue->Signal(fence.Get(), fenceVal), 0, "Failed to signal command queue!");

	return fenceVal;
}

bool CommandQueue::fenceCompleted(FenceValue fenceVal) const {
