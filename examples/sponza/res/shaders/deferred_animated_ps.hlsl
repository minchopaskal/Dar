...
struct PSInput
{
	...
	float3x3 TBN : TANGENT_MATRIX; // tangent space -> world space
	float2 uv : TEXCOORD;
};
...
{
	PSOutput psOutput;
	psOutput.albedo = IN.color;

	//psOutput.normal = float4(0.f, 1.f, 0.f, 1.f);
	//psOutput.metalnessRoughnessOcclusion = float4(0.1f, 0.0f, 0.f, 1.f);
